...
async function getInviteDetailsForDisplay(token) {
  const { rows } = await pool.query(
    `SELECT inv.email, inv.status, inv.expires_at, inv.accepted_at,
            c.id AS cohort_id, c.name AS cohort_name, i.name AS institution_name
     FROM campus_learner_invites inv
     JOIN campus_cohorts c ON c.id = inv.cohort_id
     JOIN institutions i ON i.id = c.institution_id
     WHERE inv.invite_token = $1`,
    [token]
  );
  return rows[0] || null;
}

// Strict email match (Phase 2): the invite was addressed to a specific
// person, not "whoever has the link." This also closes the single-use
// gap the V1 comment above used to flag — since users.email is unique
// (see migrate.js's users_email_unique_idx), a mismatched email can
// never belong to a second account that could "reuse" this invite after
// the rightful person already has. Re-clicking your OWN accepted invite
// stays a harmless no-op via the ON CONFLICT upsert below.
async function acceptInvite(token, userId, userEmail) {
  ...
  if (!userEmail || userEmail.trim().toLowerCase() !== invite.email.trim().toLowerCase()) {
    return { ok: false, reason: 'EMAIL_MISMATCH', invitedEmail: invite.email };
  }
...
async function listModulesWithProgress(learnerId) {
  const { rows } = await pool.query(
    `SELECT m.id, m.key, m.name, m.sequence, m.description,
            ...
            (SELECT COUNT(*)::int FROM campus_topics t WHERE t.module_id = m.id) AS topic_count
...
module.exports = {
  ...
  createLearnerInvite, getInviteByToken, getInviteDetailsForDisplay, acceptInvite, getLearnerForUser,
