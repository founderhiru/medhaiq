...
function round(n) {
  // FIX (mandatory, 2026-08-31): previously `Math.round(n || 0)` silently
  // converted null (or undefined) back into 0 -- exactly undoing the
  // "not assessed" signal avgOf()/report.overall_score now deliberately
  // produce for zero-evidence cases. null must survive this function
  // unchanged; only a real number gets rounded.
...
function avgOf(scoresData, key) {
  ...
  // scoresData array -- indistinguishable from "every scored answer
  // averaged to zero". null means "nothing was ever scored" (every turn
  // was SKIP/DONT_KNOW/SPARSE/OFF_TOPIC/NON_RESPONSIVE) -- a fundamentally
  // different fact from a real, if low, performance average.
  if (!scoresData || !scoresData.length) return null;
...
function buildCareerIntelligenceReport({ report, scoresData, questions, persona }) {
  ...
  // Response-intent formalization (approved 2026-08-13; migration 025).
  //
  // hasSubstantiveEvidence: true iff at least one real interview_scores
  // row exists for this session. This is deliberately NOT a new
  // calculation over response_intent — it reads a fact that was already
  // true or false before this feature existed (does a score row exist).
  // What changed is what CREATES that fact: routes/interview.js no
  // longer writes a score row for SKIP/DONT_KNOW turns (previously it
  // always did, with a hardcoded {star:0,...} object). Two consequences,
  // both required by the approved plan and neither special-cased here:
  //   - A NEW session where every question was SKIP/DONT_KNOW has zero
  //     score rows → hasSubstantiveEvidence = false → Not Assessed.
  //   - Any LEGACY (pre-migration) session — including one that was
  //     entirely skipped under the old behavior — still has a real score
  //     row for every question (the old code always wrote one), so
  //     hasSubstantiveEvidence is naturally true and that session's
  //     report renders exactly as it always has. This is how backward
  //     compatibility is preserved: by leaving history untouched, not by
  //     detecting "is this a legacy row."
  const hasSubstantiveEvidence = !!(scoresData && scoresData.length > 0);

  // responsePattern: deterministic counts, computed only from real
  // per-question response_intent values — never inferred, never guessed.
  // hasClassifiedIntents guards backward compatibility on the DISPLAY
  // side too: a legacy session has response_intent = NULL on every row
  // (migration 025 does not retroactively infer intent from historical
  // blank answer text, per the explicit instruction not to), so
  // responsePattern is null for it and the Response Summary line simply
  // does not render — the legacy report's layout is unchanged, not shown
  // with a fabricated or partial count.
  const hasClassifiedIntents = (questions || []).some((q) => q.response_intent != null);
  const responsePattern = hasClassifiedIntents ? {
    answered: (questions || []).filter((q) => q.response_intent === 'ANSWER').length,
    skipped: (questions || []).filter((q) => q.response_intent === 'SKIP').length,
    dontKnow: (questions || []).filter((q) => q.response_intent === 'DONT_KNOW').length,
  } : null;
  ...
  const { ranked, strengths: rankedStrengths, developmentPriorities: rankedDevelopmentPriorities } = rankVectors(fiveVectors, 2, report.improvements_json);
  const lowestVector = ranked[ranked.length - 1];
  // With zero evidence, every vector ties at 0 and rankVectors() would
  // otherwise still hand back an arbitrary "strongest"/"weakest" split —
  // exactly the "identify a lowest-scoring vector from nonexistent
  // evidence" the approved plan says not to do. Emptied out here, once,
  // rather than requiring every caller (Web/PDF/email) to remember the
  // same guard.
  const strengths = hasSubstantiveEvidence ? rankedStrengths : [];
  const developmentPriorities = hasSubstantiveEvidence ? rankedDevelopmentPriorities : [];
  ...
  return {
    ...
    // Response-intent formalization (approved 2026-08-13). hasSubstantiveEvidence
    // drives the "Interview Not Completed / Not Assessed" report state;
    // responsePattern drives the "N answered · N skipped · N don't know"
    // Response Summary line. Both null-safe / backward-compatible — see
    // the comments above where each is computed.
    ...
    // every tier, per the locked entitlement matrix. Empty arrays (not
    // omitted) when hasSubstantiveEvidence is false — see above.
    strengths,
    ...
    nextPracticeFocus: (hasSubstantiveEvidence && lowestVector)
      ? { vector: lowestVector.vector, label: lowestVector.label, score: lowestVector.score }
      : null,

    // Leadership-only narrative layer. Produced by the SAME generateReport()
    // AI call as executiveSummary/coachingInsights above — NOT a second AI
    // call (services/interview.js, REPORT_SYSTEM items 11-13). Exposed
    // unconditionally here, same pattern as strengths/developmentPriorities
    // — tier gating happens at the render layer (web/PDF/email), not here.
    // NULL-safe: report rows created before migration 022 simply have
    // these as null; callers must render "not available for this report",
    // never invent placeholder text. Not yet consumed by any route/view —
    // that is the next, separate checkpoint.
    leadershipInsights: {
      executiveInterpretation: report.executive_interpretation || null,
      roleReadiness: report.role_readiness || null,
      nextLevelDirection: report.next_level_direction || null,
    },
  };
}
