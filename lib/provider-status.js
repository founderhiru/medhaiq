...
async function fetchElevenLabsBalance() {
  // TEMPORARY diagnostic logging (2026-08-21) — added specifically to
  // determine WHY the balance check is failing in staging without
  // guessing. Safe fields only: key-exists boolean, hostname/path, HTTP
  // status, response field names present, sanitized error category. Never
  // the key itself, never the xi-api-key header value, never any response
  // field VALUE (character_count/character_limit numbers are not secrets,
  // but this logs only their presence, not their values, to keep the log
  // line minimal and consistent with "field names only"). Remove once the
  // real failure mode is confirmed from a live request.
  const hasKey = !!VOICE_SERVER_CONFIG.elevenLabsApiKey;
  const baseUrl = VOICE_SERVER_CONFIG.elevenLabsApiBaseUrl.replace(/\/$/, '');
  const path = '/v1/user/subscription';
  let hostname = null;
  try { hostname = new URL(baseUrl).hostname; } catch (_) { hostname = 'unparseable-base-url'; }

  if (!hasKey) {
    console.log('[provider-status][elevenlabs-diagnostic]', JSON.stringify({
      apiKeyExists: false, hostname, path, outcome: 'skipped — no key configured',
    }));
  ...
  try {
    ...
    if (res.status === 401 || res.status === 403) {
      console.log('[provider-status][elevenlabs-diagnostic]', JSON.stringify({
        apiKeyExists: true, hostname, path, httpStatus: res.status, outcome: 'auth rejected',
      }));
      return { balanceAvailable: false, reason: `API key rejected (HTTP ${res.status})` };
    }
    if (!res.ok) {
      console.log('[provider-status][elevenlabs-diagnostic]', JSON.stringify({
        apiKeyExists: true, hostname, path, httpStatus: res.status, outcome: 'non-2xx response',
      }));
      return { balanceAvailable: false, reason: `HTTP ${res.status}` };
    }

    const data = await res.json();
    const responseFieldNames = Object.keys(data || {});
    if (typeof data.character_limit !== 'number' || typeof data.character_count !== 'number') {
      console.log('[provider-status][elevenlabs-diagnostic]', JSON.stringify({
        apiKeyExists: true, hostname, path, httpStatus: res.status,
        responseFieldNames, outcome: 'malformed — missing character_limit/character_count',
      }));
    ...
    console.log('[provider-status][elevenlabs-diagnostic]', JSON.stringify({
      apiKeyExists: true, hostname, path, httpStatus: res.status,
      responseFieldNames, outcome: 'success',
    }));
    ...
    clearTimeout(timeout);
    const errorCategory = err.name === 'AbortError' ? 'timeout' : 'network_error';
    console.log('[provider-status][elevenlabs-diagnostic]', JSON.stringify({
      apiKeyExists: true, hostname, path, outcome: 'exception', errorCategory,
    }));
