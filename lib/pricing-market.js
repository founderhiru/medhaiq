...
// read of it here is also gated on IS_LIVE_PRODUCTION, so the cookie has
// zero effect on the real production deployment even if somehow present
// (e.g. carried over from a staging cookie during local testing against
// a prod-configured build).
...
function resolveMarket(req, user) {
  // 1. Founder staging override — never respected on real production,
  //    full stop. Same IS_LIVE_PRODUCTION flag as routes/founder.js's
  //    blockInProduction — NOT NODE_ENV, which staging also sets to
  //    'production' (see render.yaml), so checking NODE_ENV here would
  //    silently disable this override on staging too.
