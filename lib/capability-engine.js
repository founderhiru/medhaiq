...
const { getActivePackageAcquisition, getMergedCreditPool } = require('../db/package-acquisitions');
...
function visitorCapabilities() {
  ...
  return {
    ...
    package: { id: packageId, ...PRODUCT_PACKAGES[packageId], expiresAt: null },
...
async function resolveCapabilities(req) {
  ...
  if (activeAcquisition) {
    // Buy More Minutes fix: merged across ALL of this user's currently-
    // unexpired acquisitions, not just the single "active" one — a user
    // can now legitimately have more than one (e.g. Leadership + a
    // Growth top-up). See db/package-acquisitions.js's getMergedCreditPool
    // for why this is a strict generalization of the old formula (a user
    // with exactly one unexpired acquisition gets numerically IDENTICAL
    // results to before) rather than a new one.
    const mergedPool = await getMergedCreditPool(userId);
    creditsIncluded = mergedPool.totalMinutes;
    const earliestAcquiredMs = new Date(mergedPool.earliestAcquiredAt).getTime();
    sessionsForConsumption = sessions.filter(s => s.started_at && new Date(s.started_at).getTime() >= earliestAcquiredMs);
  } else if (packageId === DEFAULT_PACKAGE_ID) {
    // Anti-Abuse & Free-Offer Guardrail (2026-08): Explorer's 30 minutes
    // used to be this flat config value for ANY user with no acquisition
    // row — meaning every new signup got 30 free minutes immediately,
    // verified or not, with no server-side record of the grant ever
    // existing. That's exactly the gap config/product-packages.js's own
    // comment already flagged as the intended design ("handled at the
    // account-creation step, a package_acquisitions row with
    // source='welcome'") but that step was never actually built until
    // now (services/free-offer-guardrail.js). A real, non-legacy
    // Explorer user now always has a source='welcome' acquisition row by
    // the time they're eligible to interview — reaching this branch with
    // none means not-yet-verified, or a promotional claim that was
    // restricted as suspicious, either way correctly zero until that
    // changes. This does NOT affect Growth/Leadership (paid, always have
    // an acquisition) or founder-created accounts (db/founder-users.js
    // always calls createPackageAcquisition explicitly, source=
    // 'founder_grant').
    creditsIncluded = 0;
  ...
  return {
    // --- canonical fields (Architecture v1.5, ADR-013) ---
    package: { id: packageId, ...pkg, expiresAt: activeAcquisition ? activeAcquisition.expires_at : null },
