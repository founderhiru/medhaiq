...
async function chat(message, options = {}) {
  const response = await anthropic.messages.create({
    model: MODEL,
    max_tokens: options.maxTokens || 400, // Kept at your safe lower trial limit
    messages: [{ role: 'user', content: message }],
    // Inject system instructions with a cache block
    system: options.system ? [
      {
        type: "text",
        text: options.system,
        cache_control: { type: "ephemeral" } // Tells Claude to cache this static prompt
      }
    ] : undefined
  });
