...
const {
  acceptInvite, getInviteDetailsForDisplay, getLearnerForUser, listModulesWithProgress,
...
// GET /api/campus/invite/:token/details — unauthenticated-safe, read-only.
// Powers the invite landing page (views/campus-invite.ejs) so it can show
// institution/cohort/expiry before the person even logs in. Never
// returns anything beyond what's already visible on the page itself —
// no learner progress, no other cohort members.
router.get('/invite/:token/details', async (req, res) => {
...
// Email match is enforced in acceptInvite() itself (db/campus.js) — this
// route just supplies the authenticated user's own email, never a
// client-supplied one, so there's no way to spoof the check from here.
router.post('/join', async (req, res) => {
  ...
  const result = await acceptInvite(token, userId, user.email);
  if (!result.ok) return res.status(400).json({ error: result.reason, invitedEmail: result.invitedEmail });
